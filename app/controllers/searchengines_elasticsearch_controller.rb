...
class SearchenginesElasticsearchController < ApplicationController
  def index
    ...
    @a = "response.took: #{response.took}"
    puts @a #10 ElasticsearchTest Load (1.4ms)  SELECT "elasticsearch_tests".* FROM "elasticsearch_tests" WHERE "elasticsearch_tests"."id" = $1  [["id", 1]]
